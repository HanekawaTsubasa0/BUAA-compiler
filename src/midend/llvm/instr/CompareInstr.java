...
public class CompareInstr extends Instr {
    ...
    public String GetGvnHash() {
        ...
        CompareOp op = this.compareOp;
        if (valueL.compareTo(valueR) > 0) {
            CompareOp swapped = SwapCompareOp(op);
            if (swapped != null) {
                op = swapped;
                String tmp = valueL;
                valueL = valueR;
        ...
        return valueL + " " + op + " " + valueR;
    }

    private CompareOp SwapCompareOp(CompareOp op) {
        return switch (op) {
            case EQ, NE -> op;
            case SGT -> CompareOp.SLT;
            case SGE -> CompareOp.SLE;
            case SLT -> CompareOp.SGT;
            case SLE -> CompareOp.SGE;
        };
    }
    ...
    private void GenerateMipsCompareInstr(
        ...
        if (!IsImm16Signed(immediate)) {
            new MarsLi(Register.K1, immediate);
            this.GenerateMipsCompareInstr(registerL, Register.K1, registerResult);
            return;
        }
        switch (this.compareOp) {
    ...
    private boolean IsImm16Signed(int value) {
