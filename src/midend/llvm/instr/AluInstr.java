...
public class AluInstr extends Instr {
    ...
    public String GetGvnHash() {
        ...
        if (this.aluOp.CanChangeOrder() && valueL.compareTo(valueR) > 0) {
            String tmp = valueL;
            valueL = valueR;
            valueR = tmp;
        }
        return valueL + " " + this.aluOp + " " + valueR;
    }
    ...
    private void GenerateAluMipsInstr(Register registerL, IrConstant irConstant,
        ...
        if (!IsImm16Signed(immediate) ||
            (this.aluOp == AluType.SUB && !IsImm16Signed(-immediate))) {
            new MarsLi(Register.K1, immediate);
            this.GenerateAluMipsInstr(registerL, Register.K1, registerResult);
            return;
        }
        switch (this.aluOp) {
    ...
    private Multiplier GetMultiplier(long d, int prec) {
        int l = 32 - CountLestZero(d - 1); // l = log2(d) ?
        int post = l;
