...
public class IrFunction extends IrValue {
    ...
    public void CheckHaveReturn() {
        ...
        if (!basicBlock.LastInstrIsReturn()) {
            ...
            if (this.GetReturnType().IsInt32Type()) {
    ...
    public String toString() {
        ...
        // 参数声明
        builder.append("(");
        builder.append(this.parameterList.stream().map(IrParameter::toString).collect(Collectors.joining(", ")));
        builder.append(") {\n");
        // 语句声明
        builder.append(this.basicBlockList.stream().map(IrBasicBlock::toString).collect(Collectors.joining("\n")));
        builder.append("\n}");
